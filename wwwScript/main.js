...
function isEnemy(obj) {
    return ENEMY_TYPES.some(function(Type) {
    ...
    update: function(_, timestamp) {
        ...
        if (toDie.length) {
            ...
            this.score += toDie.map(function(cmp) {
                if (cmp instanceof Whirrer) {
                    return 5;
                }
                if (cmp instanceof Dasher) {
                    return 3;
                }
                return 1;
        ...
        var hits = game.getComponents(function(cmp) {
            return isEnemy(cmp) && cmp.position.clone().substract(player.position).length() < PLAYER_DISTANCE;
        });
